<script lang="ts">
    import { devTools } from '$lib/stores/DevTools.svelte.ts';
...
{#if isMounted && devTools.isDevMode}
    <div class="devtools-wrapper">
        <button class="devtools-trigger" onclick={devTools.togglePanel}> devTools </button>
            ...
            <div class="devtools-panel">
                ...
                {#if devTools.activeTab === 'general'}
                    <div class="tab-content">
                        <div class="setting-row">
                            ...
                            <button
                                class="toggle-btn {devTools.isDevMode ? 'on' : 'off'}"
                                onclick={devTools.toggleDevMode}
                            >
                        ...
                        <div class="setting-block">
                            ...
                            <div class="inline-input">
                                ...
                                />
                                <button class="action-btn" onclick={devTools.applyCustomRemembrance}
                                    >Set</button
                                >
                            </div>
                            <div class="btn-group">
                                <button
                                    class="action-btn unlimited"
                                    onclick={devTools.toggleUnlimitedRemembrance}
                                >
                                    {devTools.customDuration === 'Forever'
                                        ? '⏳ Standard Duration'
                                        : '♾️ Set Unlimited'}
                                </button>
                ...
                {:else if devTools.activeTab === 'storage'}
                    <div class="tab-content">
                        <div class="flex-between">
                            ...
                            <button class="danger-btn" onclick={devTools.clearAllStorage}
                                ...
                                <div class="storage-item">
                                    <div class="storage-inputs">
                                        ...
                                        <input
                                            ...
                                            onchange={() =>
                                                devTools.updateStorageItem(item.key, item.value)}
                                            class="val-input"
                                        />
                                    </div>
                                    <button
                                        class="del-btn"
                                        onclick={() => devTools.removeStorageItem(item.key)}
                                        >✕</button
                                    >
                                </div>
                            {/each}
                ...
                {:else if devTools.activeTab === 'inject'}
                    <div class="tab-content">
                        ...
                        <textarea
                            id="script-textarea"
                            bind:value={devTools.scriptInput}
                            placeholder="console.log(window.location.href);"></textarea>
                        <button class="action-btn full" onclick={devTools.runScript}
                            >▶ Run Script</button
                        >
                    </div>
                {/if}
...
{#if isMounted && devTools.isPreviewOpen}
    ...
    >
        <div class="drag-header" role="toolbar" tabindex="0" onmousedown={devTools.startDrag}>
            ...
            <div class="window-actions">
                <button
                    class="control-btn"
                    onclick={() => window.open(devTools.previewUrl, '_blank')}>↗</button
                >
